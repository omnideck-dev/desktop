// Downloads and checksum-verifies the pinned `omnideck` CLI sidecar
// binaries listed in `src-tauri/binaries/vendor-manifest.json`, writing
// `src-tauri/binaries/omnideck-<target-triple>[.exe]` for each. Model:
// omnideck/desktop's scripts/fetch-sidecars.mjs (sibling repo) — same
// mechanism, adapted to this repo's externalBin naming (`omnideck`, not
// `omnideck-cli`; see tauri.conf.json's `bundle.externalBin`).
//
// Usage: node scripts/fetch-sidecars.mjs [targetTriple ...]
// With no args, fetches every target in the manifest.
//
// OMNIDECK_CLI_ARCHIVE_DIR may point at a directory of pre-downloaded
// release archives + SBOMs (same filenames as the GitHub release assets)
// for an offline/sandboxed build. The pinned hashes are still enforced.
...
function filename(targetTriple) {
  return `omnideck-${targetTriple}${targetTriple.includes('windows') ? '.exe' : ''}`;
}
