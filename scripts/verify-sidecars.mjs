// Re-checksums already-fetched CLI sidecar binaries + SBOMs against
// `src-tauri/binaries/vendor-manifest.json` without re-downloading anything
// — a cheap re-verification step for CI or local dev. Model: omnideck/desktop's
// scripts/verify-sidecars.mjs (sibling repo), adapted to this repo's
// `omnideck-<target-triple>` naming (see fetch-sidecars.mjs).
//
// Usage: node scripts/verify-sidecars.mjs [targetTriple ...]
...
function filename(targetTriple) {
  ...
  return `omnideck-${targetTriple}${extension}`;
}
...
const actualFiles = (await readdir(binaryRoot)).filter((entry) => entry.startsWith('omnideck-'));
