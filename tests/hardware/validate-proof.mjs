// Validates a packaged-build smoke proof against the pinned CLI manifest —
// ported from the sibling repo's tests/hardware/validate-proof.mjs, adapted
// to this repo's floor-checked (not exact-matched) version policy: the
// proof's cliVersion only needs to be >= vendor-manifest's pinned version in
// spirit, but since the proof comes from the exact bundled binary the
// manifest pinned, an exact match is what a correct build actually
// produces — a mismatch here means the wrong binary got bundled, which is
// exactly the build-mistake case the floor check exists to catch (see
// cli_bridge.rs's MINIMUM_CLI_VERSION doc comment).
...
assert.equal(
  proof.cliVersion,
  vendorManifest.version,
  "packaged sidecar version does not match the vendor manifest",
);
assert.equal(
  proof.cliCommit,
  vendorManifest.commit,
  "packaged sidecar commit does not match the vendor manifest",
);
