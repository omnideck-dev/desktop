// Static release-artifact contract: asserts a release directory contains
// exactly the expected installer set for this repo's 3-target build matrix
// (see .github/workflows/release.yml), with correct per-format magic bytes
// and matching checksums — not just "some files exist"
// (`fail_on_unmatched_files: true` on the publish step already catches a
// totally missing platform; this catches a subtly wrong one, e.g. a
// corrupted or empty artifact, or a checksum that doesn't match its file).
//
// Ported from the sibling repo's tests/releasecontract/verify-release.mjs,
// scaled from its 10-artifact/6-target matrix down to this repo's 5-artifact/
// 3-target one (see reference/desktop-hardening-migration-PLAN.md's Phase 6
// — deliberately scoped down from the sibling's linux/windows/macos ×
// x64/arm64 matrix to one target per OS).
...
export async function verifyReleaseDirectory({ directory, version }) {
  assert.match(version, VERSION_PATTERN, "version must be a SemVer tag such as v0.5.0-alpha.2");
  ...
  assert.deepEqual(
    ...
    [...expectedNames].sort(),
    "published desktop package matrix does not match the supported five artifacts",
  );
